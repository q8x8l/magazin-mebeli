    ...
    foreach ($m1 as $key => $value) {
      ...
      switch ($клас) {
        ...
        case 'категории_фильтра_каталога':
          ...
          // Пропустим потомки
            // if ($путь == implode ('/', array_slice (explode ('/', $value), 0, count (explode ('/', $путь)) ))) continue;
            // echo $путь;
            // echo '<br>';
            // echo implode ('/', array_slice (explode ('/', $value), 0, count (explode ('/', $путь))));
            // echo '<br>';
          echo '<p индекс="' . ($key - 1) . '" метка="' . $метка [1] . $потомок . '" путь="' . $value . '/">';
            echo $последняя_папка;
            // echo '<a href="' . $домен . 'каталог.php?путь=' . $value . '&страница=1">>></a>';
          echo '</p>';
          // $массив_индексов [] = $key - 1;
        break;
      }
    }
    // if ($клас == 'категории_фильтра_каталога')
    //   {
    //     echo '<script class="массив_индексов">';
    //       foreach ($массив_индексов as $key => $индекс) {
    //         echo 'console.log ("' . $индекс . '");';
    //       }
    //       echo '$ (".массив_индексов").remove ();';
    //     echo '</script>';
    //   }
  }
