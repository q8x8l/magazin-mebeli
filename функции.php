<?php
      ...
      foreach ($массив_папок as $k => $вложенная_папка)
        {
          ...
          if ($глубина != '')
            if (count (explode ('/', $папка)) > $глубина + 1) continue;
  ...
  function вывод_категорий ($каталог_для_меню, $клас, $путь, $внутренний_запрос = 0, $уровень_запроса_в_базу = 1) {
    // удалю из меню товары
      $m0 = [];
              ...
              switch ($внутренний_запрос)
                {
                  case 0:
                    ...
                    // Передадим метку параметром в функцию
                      // Ограничю вывод потомков
                      if ($метка [1] > $уровень_запроса_в_базу) continue;
                      echo '<p индекс="' . $key . '" метка="' . $метка [1] . $потомок . '" путь="' . $папка . '/">';
                        echo '<a href="">' . $последняя_папка . '</a>';
                      echo '</p>';
                  break;
                  case 1:
