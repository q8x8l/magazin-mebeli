<?php
  ...
  вывод_категорий                 ($каталог_для_меню, $клас = 'категории_фильтра_каталога', $_GET ['путь'], $внутренний_запрос = 1);
  ...
  foreach (explode ('^__^', $каталог) as $позиция_каталога) {
    ...
    // Отфильтруем массив
      ...
      if ($_GET ['индекс_фильтра'] == 2) {
        if ($_GET ['флаги'] [0] == 1) if (!str_contains ($ячейка [1], $микро_запросы_к_меткам_товара [1][0])) continue;
        if ($_GET ['флаги'] [0] == 0) if ( str_contains ($ячейка [1], $микро_запросы_к_меткам_товара [1][0])) continue;
        if ($_GET ['флаги'] [1] == 1) if (!str_contains ($ячейка [1], $микро_запросы_к_меткам_товара [1][1])) continue;
        if ($_GET ['флаги'] [1] == 0) if ( str_contains ($ячейка [1], $микро_запросы_к_меткам_товара [1][1])) continue;
      }
  ...
  if ($_GET ['индекс_фильтра'] != 3) {
    $цифры_категории = [];
    foreach (array_unique ($доступные_папки_на_вывод) as $key => $значение_цикла_0) {
