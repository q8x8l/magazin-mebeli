<?php
  ...
  if ($_GET ['фильтр'] == 1)
    {
      ...
      foreach ($массив_для_внутреннего_запроса as $ключ => $значение_цикла_0)
        {
          if (in_array ($ключ, $_GET ['дерево'])) $массив_категорий_в_выброс [] = $массив_для_внутреннего_запроса [$ключ];
        }
      $отфильтрованный_массив_брендов = [];
      foreach ($_GET ['бренды'] as $цифра_бренда_в_трафике)
        {
  ...
  // отдадим трафик
    foreach (array_unique ($уникальный_массив_каталога_в_ответ) as $k => $строка_в_ответ) {
      ...
      echo $пункт_запятая . $строка_в_ответ . '^_^' . $k;
    }
    echo ';;';
    echo $индекс;
  echo ';;';
