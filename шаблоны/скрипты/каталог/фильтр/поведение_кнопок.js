	...
	console.log (
		`
			Сделаю клик по ви на дереве:
				сделаю тоггл потомков при клике на родительскую ветку при открытом вылете веток
		`
	);
	function пробег_по_дереву_и_чека_по_потомкам (парент)
		{
			(ветки = $(панель + '.чекбоксы.дерево p')).each (function ()
				{
					...
					switch (парент.children ().eq (1).attr ('class'))
						{
							case 'v a':
								tag_v.addClass 	  ('a');
								break;
			...
			switch (парент.attr ('class'))
				{
					...
					case 'есть_ветка выпал':
						...
						пробег_по_дереву_и_чека_по_потомкам (парент);
						// Запрет на вылет
...
// Событие чека по элементам дерева
	$('body').on ('click', панель + (дерево = '.чекбоксы.дерево') + ' p i.v', function ()
		{
			пробег_по_дереву_и_чека_по_потомкам ($(this).parent ());
