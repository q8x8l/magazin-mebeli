if ($(панель = '.панель_сортировки_и_фильтра ').length == 1) {
  ...
  function аякс_в_ленту_каталога (путь, страница, бренды, сортировка, клик_по_фильтру) {
    url = new URLSearchParams (window.location.search);
    возьмем_массив_из_чекеров ();
  ...
  function щет (аякс = 0, aa) {
    $.get (домен + локальный_путь_к_файлу + 'фильтр/щет.php', {
      ...
      $.get (домен + локальный_путь_к_файлу + 'пагинация.php', {
        ...
        console.log (D.split ('qqqq')[0])
        $('.Пагинация').html (D);
        $('.Пагинация a').each (function ()
          {
            $(this).attr ('href', $(this).attr ('href') + '&фильтр=' + фильтр + сортировка);
          }
        );
      });
    });
