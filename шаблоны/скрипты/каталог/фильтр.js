if ($(панель = '.панель_сортировки_и_фильтра ').length == 1) {
  ...
  // Тест аякса на ленту ленту каталога
    // function аякс_в_ленту_каталога (путь, страница, дерево, бренды, сортировка) {
    function аякс_в_ленту_каталога (путь, страница, бренды, сортировка) {
      url = new URLSearchParams (window.location.search);
      возьмем_массив_из_чекеров ();
      $.get (домен + локальный_путь_к_файлу + 'лента_каталога.php', {
        ...
        функция_ленты_каталога (D.split (';;') [0].slice (0, -2).split (', '));
        if (количество_элементов = D.split (';;') [1]) {
          поставим_блок_пагинации (количество_элементов, страница = url.get ('страница'), путь = url.get ('путь'), фильтр = url.get ('фильтр'));
        }
        щет (0);
      })
    }
