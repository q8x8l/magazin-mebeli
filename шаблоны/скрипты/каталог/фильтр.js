if ($(панель = '.панель_сортировки_и_фильтра ').length == 1) {
        ...
        фильтр = фильтр.split (';');
          флаги = фильтр [0].slice (1, -1).split (',')
            зарос = панель + '.toggleText';
              $(зарос + '.tT0 > p').eq (0).text (флаги [0]);
              $(зарос + '.tT1 > p').eq (0).text (флаги [1]);
  ...
  function запрос_фильтра (индекс_фильтра, сброс) {
    ...
    $.get (домен + локальный_путь_к_файлу + 'фильтр/динамика.php', {
      ...
      // console.log (ответ_от_сервера.split (';') [3].slice (0, -1).split (','));
      if (индекс_фильтра != 9) {
        ...
        if (индекс_фильтра != 2) {
          флаги = ответ_от_сервера.split (';')[2].split (',');
            флаги [0] = флаги [0];
            флаги [1] = флаги [1];
          $(панель + '.toggleText.tT0 > p').eq (0).text (флаги [0]);
          $(панель + '.toggleText.tT1 > p').eq (0).text (флаги [1]);
        }
        if (индекс_фильтра != 3) {
        ...
        параметры =
          ...
          ';' +
            (бренды = '[' + цифры_бренды + ']') +
          ';' +
            (цифры_категории = '[' + массив_ответов_от_сервера [ключ_папки_на_вывод] + ']') +
          ';' +
            сортировка;
