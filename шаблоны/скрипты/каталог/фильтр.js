if ($(панель = '.панель_сортировки_и_фильтра ').length == 1) {
  ...
  // Выпадение подкатегорий
    ...
    console.log
      (
        `
          1 - Вылеты интервально блоков "Товар"
          2 - Копируем цифровую механику из ленты каталога в динамику и пагинацию
          3 - И удалим функцию обработки тегов на категории и бренды
          4 - Добавим функцию загрузки каталогов-потомков
          5 - Добавим щетчик к брендам и категориям
        `
      )
    ;
}
