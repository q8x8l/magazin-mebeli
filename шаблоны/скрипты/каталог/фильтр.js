if ($(x1 = '.панель_сортировки_и_фильтра ').length == 1) {
  i2 = 'блоки/каталог/'
  // Вывод категорий в фильтр
    function выведем_массивом_категории (массив_для_категорий_фильтра) {
      массив_для_категорий_фильтра.forEach (function (item) {
        if (Array.isArray(item)) {
          выведем_массивом_категории (item)
        }
        else if (typeof(item) == 'object') {
          ключ = Object.keys(item)[0]
          if (ключ != undefined) {
            элемент = ключ.split(':')
            c = элемент[0].split('/').length
            $(x1 + '.чекбоксы_категорий').append ('<span путь="' + элемент[0] + '/" style="margin-left: ' + ((c - 1) * 20 - 20) + 'px">' + элемент[1] + '</span>')
    ...
    выведем_массивом_категории (массив_для_категорий_фильтра)
    $(x1 + '.чекбоксы_категорий script').remove ()
  // Конец вывода категорий в фильтр
    ...
    if ((фильтр = x.get('фильтр')) != null)
      {
        фильтр = фильтр.split(';')
          ...
          // Прочекаем цифровое_дерево категорий в фильтре
    ...
    // Удалим корни
      полное_дерево.forEach((item, i) => {
        ...
        if (следующий != undefined) {
          if (item.length < следующий.length) {
            delete полное_дерево [i]
          }
        }
      })
  }
